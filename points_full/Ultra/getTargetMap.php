...
function getTargetMap($arIds,$startYear)
{
	...
	$targetObjects = mysqli_query($con,"SELECT ar_id, target, payment_perc,rate,multiplier, year,month FROM target WHERE  Year >='$startYear' AND target >0 AND ar_id IN('$arIds')") or die(mysqli_error($con));		 
