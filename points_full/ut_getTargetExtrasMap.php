...
function getTargetExtrasMap($arIds,$startYear)
{
	...
	$targetExtrasMap = null;
	
	$targetExtrasObjects = mysqli_query($con,"SELECT ar_id, qty, YEAR(date), MONTH(date) FROM targetbags_ultra WHERE  YEAR(date) >='$startYear' AND ar_id IN('$arIds')") or die(mysqli_error($con).' LINE 8');
