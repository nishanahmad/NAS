...
if(isset($_SESSION["user_name"]))
{
	...
	require 'ut_getTargetMap.php';
	require 'ut_getTargetExtrasMap.php';
	require 'ut_getSaleMap.php';
	require 'ut_getBoosterMap.php';	
	require 'ut_getSpecialTargetMap.php';	
	require '../SpecialTarget/ut_dropDownGenerator.php';
	require '../Target/functions/latestYear.php';
	require '../Target/functions/latestMonth.php';
	
	$latestYear = getLatestYearUltra($con);
	$latestMonth = getLatestMonthUltra($con);
	
	$latestYearRedemptionQuery = mysqli_query($con, "SELECT DISTINCT YEAR(date) FROM redemption_ultra ORDER BY YEAR(date) DESC LIMIT 1") or die(mysqli_error($con).' LINE 20');		
	...
	if($latestYearRedemption > $latestYear)
	{
		...
		$latestMonthRedemptionQuery = mysqli_query($con, "SELECT DISTINCT MONTH(date) FROM redemption_ultra WHERE YEAR(date) = $latestYear ORDER BY date DESC LIMIT 1") or die(mysqli_error($con).' LINE 25');		
	...
	{
		$latestMonthRedemptionQuery = mysqli_query($con, "SELECT DISTINCT MONTH(date) FROM redemption_ultra WHERE YEAR(date) = $latestYear ORDER BY date DESC LIMIT 1") or die(mysqli_error($con).' LINE 30');		
	...
	$yearList1 = mysqli_query($con, "SELECT DISTINCT year FROM target_ultra ORDER BY year DESC") or die(mysqli_error($con).' LINE 63');	
	$yearList2 = mysqli_query($con, "SELECT DISTINCT YEAR(date) FROM redemption_ultra ORDER BY YEAR(date) DESC") or die(mysqli_error($con).' LINE 64');	
	...
	{
		$months = array();
		$monthList1 = mysqli_query($con, "SELECT DISTINCT month FROM target_ultra WHERE year = $yearObj ORDER BY month ASC" );
		foreach($monthList1 as $monthObj)
			$months[] = $monthObj['month'];

		$monthList2 = mysqli_query($con, "SELECT DISTINCT MONTH(date) FROM redemption_ultra WHERE YEAR(date) = $yearObj ORDER BY MONTH(date) ASC" );
	...
	$arObjects =  mysqli_query($con,"SELECT id,name,mobile,whatsapp,shop_name,sap_code FROM ar_details WHERE Type != 'Engineer' ORDER BY name ASC ") or die(mysqli_error($con).' LINE 101');		 
	...
	if($dateString == 'FULL')
	{	
		$targetObjects = mysqli_query($con,"SELECT ar_id, target, payment_perc,rate,multiplier FROM target_ultra WHERE  month = '$month' AND Year='$year' AND target > 0 AND ar_id IN('$arIds')") or die(mysqli_error($con));		 
		...
		$sales = mysqli_query($con,"SELECT ar_id,SUM(qty),SUM(return_bag) FROM nas_sale WHERE 
									deleted IS NULL AND '$year' = year(`entry_date`) AND '$month' = month(`entry_date`) 
									AND ar_id IN ('$arIds') AND
									product IN (SELECT Id FROM products WHERE brand = 'UT' AND category = 'CEMENT')
									GROUP BY ar_id") or die(mysqli_error($con).' LINE 131');	
		foreach($sales as $sale)
		{
			$arId = $sale['ar_id'];
			$targetBagsQuery = mysqli_query($con,"SELECT SUM(qty) FROM targetbags_ultra WHERE '$year' = year(`date`) AND '$month' = month(`date`) AND ar_id = $arId") or die(mysqli_error($con).' LINE 135');
	...
	else
	{
		...
		$boostQuery = mysqli_query($con,"SELECT * FROM special_target_booster_ultra WHERE  fromDate = '$fromDate' AND toDate = '$toDate'") or die(mysqli_error($con).' LINE 179');		 
		$boost = mysqli_fetch_array($boostQuery,MYSQLI_ASSOC);
		
		$specialTargetObjects = mysqli_query($con,"SELECT ar_id, fromDate, toDate,special_target FROM special_target_ultra WHERE  toDate = '$toDate' AND fromDate = '$fromDate' AND special_target >0 AND ar_id IN('$arIds')") or die(mysqli_error($con));		 
		foreach($specialTargetObjects as $specialTarget)
		{
			...
			$sales = mysqli_query($con,"SELECT ar_id,SUM(qty),SUM(return_bag) FROM nas_sale WHERE 
										deleted IS NULL AND entry_date >= '$start' AND entry_date <= '$end' AND 
										ar_id = '$arId' AND 
										product IN (SELECT Id FROM products WHERE brand = 'UT' AND category = 'CEMENT')
										GROUP BY ar_id") or die(mysqli_error($con).' LINE 192');	
			foreach($sales as $sale)
			{
				$total = $sale['SUM(qty)'] - $sale['SUM(return_bag)'];
				$totalWithExtra = $total;
				$extraBags = mysqli_query($con,"SELECT ar_id,SUM(qty) FROM extra_bags_ultra WHERE date >= '$start' AND date <= '$end' AND ar_id = '$arId' GROUP BY ar_id") or die(mysqli_error($con).' LINE 197');	
	...
	$currentRedemption = mysqli_query($con,"SELECT ar_id,SUM(points) FROM redemption_ultra WHERE '$year' = year(`date`) AND '$month' = month(`date`) AND ar_id IN ('$arIds') GROUP BY ar_id") or die(mysqli_error($con).' LINE 221');	
...
function getPrevPoints($arList,$endYear,$endMonth,$dateString)
{
	...
	if($dateString != 'FULL')
		$stDates = mysqli_query($con,"SELECT from_date,to_date FROM special_target_date_ultra WHERE to_date < '$endDate' AND from_date > '2018-01-01' ") or die(mysqli_error($con).' LINE 492');	
	else
		$stDates = mysqli_query($con,"SELECT from_date,to_date FROM special_target_date_ultra WHERE to_date <= '$endDate' AND from_date > '2018-01-01' ") or die(mysqli_error($con).' LINE 494');	
	...
	foreach($stDates as $stDate)
	{
		...
		$sales = mysqli_query($con,"SELECT ar_id,SUM(qty),SUM(return_bag) FROM nas_sale WHERE 
									...
									ar_id IN ('$arIds') AND 
									product IN (SELECT Id FROM products WHERE brand = 'UT' AND category = 'CEMENT')
									GROUP BY ar_id") or die(mysqli_error($con).' LINE 504');	
		foreach($sales as $sale)
		{
			$arId = $sale['ar_id'];
			$total = $sale['SUM(qty)'] - $sale['SUM(return_bag)'];
			$totalWithExtra = $total;
			$extraBags = mysqli_query($con,"SELECT ar_id,SUM(qty) FROM extra_bags_ultra WHERE date >= '$start' AND date <= '$end' AND ar_id = '$arId' GROUP BY ar_id") or die(mysqli_error($con).' LINE 510');	
			foreach($extraBags as $extraBag)
				$totalWithExtra = $totalWithExtra + $extraBag['SUM(qty)'];
	...
	$redemptionList = mysqli_query($con,"SELECT ar_id,SUM(points) FROM redemption_ultra WHERE  ( (YEAR(date) = '$endYear' AND MONTH(date) < '$endMonth') OR (YEAR(date) < '$endYear')) AND ar_id IN('$arIds') GROUP BY ar_id") or die(mysqli_error($con));		 	
